...
describe('assertIsEvent', () => {
    ...
    const createEvents = () => {
      ...
      const tryPush = (ctor, name) => {
        try {
          ...
          console.warn(`[SKIPPED] ${ctor.name} not supported in this environment: ${err.message}`);
        }
      };
      ...
      if (typeof ClipboardEvent !== 'undefined') tryPush(ClipboardEvent, 'copy');
  ...
  describe('invalid non-event values', () => {
    ...
    test.each(invalidValues)('throws TypeError for %p', (value) => {
      ...
      expect(() => assertIsEvent(value, 'arg')).toThrow(/arg.*instance of Event/i);
    });
  });
});
