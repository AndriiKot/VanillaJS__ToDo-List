...
describe('assertIsHTMLTagElement', () => {
  describe('valid HTML elements (should NOT throw)', () => {
    test.each(VALID_STANDARD_HTML_TAGS)('does not throw for a valid <%s> element', (tagName) => {
  ...
  describe('invalid values (should throw TypeError)', () => {
    ...
    test.each(invalidValues)('throws TypeError for %p', (value) => {
      ...
      expect(call).toThrow(
        new RegExp(`Expected value to be an instance of HTMLElement, but received .* of type .*`),
      );
    });
  });
});
