...
describe('assertIsHTMLTagElement', () => {
  test('throws TypeError if not an HTMLElement', () => {
    expectTypeErrorMessage(() => assertIsHTMLTagElement({}), 'value', 'an instance of HTMLElement');
  });

  test('does not throw if HTMLElement', () => {
    expect(() => assertIsHTMLTagElement(document.createElement('div'))).not.toThrow();
  });
});
...
describe('setTextContent', () => {
  ...
  test('throws TypeError if text is not a string', () => {
    ...
    for (const value of invalidTexts) {
      expectTypeErrorMessage(() => setTextContent(div, value), 'second argument', 'string');
    }
  });
});
